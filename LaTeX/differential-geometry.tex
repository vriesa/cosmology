...
The set $\mathscr{D}(\mathfrak{A})$ of all charts $(U, h)$ of $X$ changing differentiably then is an $n$-dimensional maximal atlas of $X$. \cite[2]{Jaenich-1992}
...
This definition can be formulated more precisely by identifying functions which coincide on a neighborhood of $p$: two function $f$ and $g$ on $\mathscr{M}$ being differentiable at $p$ have the same \emph{germ}\index{germ} at $p$ if there exists a neighborhood of $p$ where they coincide. The equivalence class of differentiable functions at $p$ which have the same germ as a function $f$ is called a \emph{germ} of $f$. The disjoint equivalence classes are called the \emph{germs of differentiable functions} at $p$. Germs form an algebra.
A \emph{tangent vector} then is a derivation on the algebra of of germs of differentiable functions at $p$.
...
\begin{beispiel}
	...
	An illustrative example of a two-dimensional manifold is the surface $S^2 (r)$ of a three-dimensional sphere of radius $r$ in $\mathbb{R}^3$,
	\begin{equation}
...
\item
A topological group $G$ of homeomorphisms $F \to F$, called the \emph{structure group}, consisting of the homeomorphisms $\hat{\varphi}_{i,p} \circ \hat{\varphi}_{j,p}^{-1}: F \to F$ for any $p \in U_i \cap U_j \subset B$.
The mappings $f_{ij}: U_i \cap U_j \to G$ induced by $p \mapsto f_{ij}(p) = \hat{\varphi}_{i,p} \circ \hat{\varphi}_{j,p}^{-1}$ are called its \emph{transition functions}\index{transition function of a fiber bundle}.

\end{itemize}
Often, a fiber bundle is simply written as the short sequence 
...
\begin{beispiel}
	...
	Let $\mathscr{M} \subseteq \mathbb{R}^3$ be open. Then the vector-valued 1-form $\DD \fett{s}$ and the vector-valued 2-form $\DD \fett{S}$,
	\begin{equation}
		\DD \fett{s}
		= \left(\begin{array}{@{\,}c@{\,}}
	...
	Note moreover that the line element $\DD s = \sqrt{\DD x^2 + \DD y^2 + \DD z^2}$, which is applied to compute the arc length of a curve in $\mathbb{R}^3$ and is used, in a more general form, in differential geometry, is \emph{not} a 1-form. \cite[68]{Jaenich-1992}  
\end{beispiel}
...
\begin{beispiel}
	...
	Let be $\mathscr{M} \subseteq \mathbb{R}^3$.
...
\begin{beispiel}
	...
	Note that the speed of light $c$ then is given by
	\begin{equation}
...
%		\Omega^{n-1} (\mathscr{M}, T\mathscr{M}) \stackrel{\D^\nabla}{\longrightarrow}
	...
	operator $\DD^\nabla$ depends on the curvature of the manifold, but does not satisfy the Poincar\'e rule in general, i.e., we may have $\DD^\nabla \circ \DD^\nabla \ne 0$.
	\cite[252]{Jaenich-2002}
	More precisely, the Cartan structure equations in a semi-Riemannian manifold yield the relation
	\begin{equation}
		\DD^\nabla \theta^i 
	...
	and $\gamma^i_{kj}$ are the connection coefficients given by the unique torsion-free linear connection $\nabla$ through the covariant derivation\index{covariant derivation} of $e_i$, $\nabla e_i = \gamma^j_{ki} \theta^k \otimes e_j$, where $\{e_i\}$ and $\{\theta^i\}$ are dual frames.
